...
export const fileController = {
    ...
    uploadFile: asyncHandler(async (req, res) => {
        ...
        const { storageKey, size } = await storageService.saveDraftFile(req.file.buffer, studentProfile._id.toString(), "general", // Not submission-specific
     ...
     * 🔒 Student/Alumni only
     */
    uploadDraft: asyncHandler(async (req, res) => {
        ...
        if (!["submitted", "resubmission"].includes(submission.status)) {
            throw new BadRequestError(`Cannot upload draft in status: ${submission.status}. Only allowed in 'submitted' or 'resubmission' status.`);
        }
        ...
        const { storageKey, size } = await storageService.saveDraftFile(req.file.buffer, studentProfile._id.toString(), submissionId, req.file.originalname);
     ...
     * 🔒 Faculty only
     */
    uploadFinal: asyncHandler(async (req, res) => {
        const submissionId = req.params.submissionId;
        const actorUserId = req.user.id;
        const actorRole = req.user.role;
        // 1. Validate file presence
        ...
        if (submission.status !== "approved") {
            throw new BadRequestError(`Cannot upload final LoR. Submission must be in 'approved' status, currently: ${submission.status}`);
        }
        ...
        if (existingFinal) {
            throw new BadRequestError("Final LoR already uploaded for this submission. Delete existing one first if you need to replace it.");
        }
        // 6. Save file to storage
        const { storageKey, size } = await storageService.saveFinalFile(req.file.buffer, facultyProfile._id.toString(), submissionId, req.file.originalname);
        // 7. Create File document
        const fileDoc = await File.create({
            submissionId: new Types.ObjectId(submissionId),
            type: "final",
        ...
        await submissionService.updateStatus(submissionId, actorUserId, actorRole, {
            newStatus: "completed",
