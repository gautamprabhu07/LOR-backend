 ...
 */
export const getEmployment = async (userId) => {
...
export const updateEmployment = async (userId, data) => {
    ...
    const profile = await StudentProfile.findOneAndUpdate({ userId, isActive: true }, { $set: { employment: data } }, { new: true, runValidators: true });
    if (!profile) {
        throw new NotFoundError("Student profile not found");
    }
    return profile.employment;
 ...
 */
export const getTargetUniversities = async (userId) => {
    const profile = await StudentProfile.findOne({
        userId,
        isActive: true
    }).lean();
...
export const addTargetUniversity = async (userId, data) => {
    ...
    profile.targetUniversities.push(data);
    await profile.save();
    return profile.targetUniversities[profile.targetUniversities.length - 1];
};
export const deleteTargetUniversity = async (userId, targetId) => {
    const profile = await StudentProfile.findOne({ userId, isActive: true });
    if (!profile) {
        throw new NotFoundError("Student profile not found");
    }
    const initialLength = profile.targetUniversities.length;
    profile.targetUniversities = profile.targetUniversities.filter((target) => target._id.toString() !== targetId);
...
export const addCertificate = async (userId, data) => {
    ...
    if (!file) {
        throw new ForbiddenError("File not found or you don't have permission to access it");
    }
...
export const deleteCertificate = async (userId, certificateId) => {
    ...
    profile.certificates = profile.certificates.filter((cert) => cert._id.toString() !== certificateId);
