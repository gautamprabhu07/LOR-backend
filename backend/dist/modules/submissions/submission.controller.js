...
export const submissionController = {
    ...
    create: asyncHandler(async (req, res, next) => {
        ...
        // 3. Service call
        const submission = await submissionService.createSubmission(req.user.id, req.user.role, {
    ...
    list: asyncHandler(async (req, res, next) => {
        ...
        // 3. Service call
        const submissions = await submissionService.listSubmissions(req.user.id, req.user.role, {
            status: query.status,
    ...
    getById: asyncHandler(async (req, res, next) => {
        ...
        const submission = await submissionService.getSubmission(String(req.params.id), req.user.id, req.user.role);
        // 3. Response
        res.status(200).json({
            status: "success",
            data: {
    ...
    updateStatus: asyncHandler(async (req, res, next) => {
        ...
        const submission = await submissionService.updateStatus(String(req.params.id), req.user.id, req.user.role, dto);
        // 4. Response
        res.status(200).json({
            status: "success",
            data: {
    ...
    delete: asyncHandler(async (req, res, next) => {
        ...
        // 2. Service call
        await submissionService.deleteSubmission(String(req.params.id), req.user.id, req.user.role);
        // 3. Response
        res.status(200).json({
            status: "success",
