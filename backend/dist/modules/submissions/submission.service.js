...
import { statusTransitions } from "../../core/utils/statusTransitions.js";
...
export const submissionService = {
     ...
     */
    async createSubmission(actorUserId, actorRole, dto) {
        // 1. Role check
        ...
        if (existingSubmission) {
            throw new BadRequestError("Active submission already exists with this faculty. Complete or delete existing submission first.");
        }
     ...
     */
    async listSubmissions(actorUserId, actorRole, filters) {
     ...
     */
    async getSubmission(submissionId, actorUserId, actorRole) {
     ...
     */
    async updateStatus(submissionId, actorUserId, actorRole, dto) {
        // 1. Get submission
        const submission = await Submission.findById(submissionId);
        if (!submission || !submission.isActive) {
            throw new NotFoundError("Submission not found");
     ...
     */
    async deleteSubmission(submissionId, actorUserId, actorRole) {
 ...
 */
async function validateOwnership(submission, actorUserId, actorRole) {
    // Admin bypass
    if (actorRole === "admin")
        return;
    const { studentUserId, facultyUserId } = await getSubmissionUserIds(submission);
    const isOwner = (actorRole === "student" || actorRole === "alumni") && actorUserId === studentUserId ||
 ...
 */
async function getSubmissionUserIds(submission) {
 ...
 */
async function sendStatusChangeEmail(submission, newStatus, oldStatus, remark) {
    try {
        const [studentProfile, facultyProfile, studentUser, facultyUser] = await Promise.all([
            ...
            StudentProfile.findById(submission.studentId).then(sp => sp ? User.findById(sp.userId) : null),
            FacultyProfile.findById(submission.facultyId).then(fp => fp ? User.findById(fp.userId) : null)
        ]);
